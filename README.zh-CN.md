...
<div align="center">
  <img src="src/运行截图.png" alt="主界面" width="600"/>
  <p><em>主界面</em></p>
</div>

<div align="center">
  <img src="src/测试结果.jpg" alt="测试结果" width="600"/>
  <p><em>测试结果</em></p>
</div>
...
- 🌍 **出口 IP 识别** — 测试后查询真实出口 IP 位置，准确重命名节点
...
- **建议值：16-64** — 过高可能导致测试结果不准确
- 并发过高容易触发目标服务器限流或本地资源耗尽
- 大量节点（500+）建议在设置中启用分批模式

### 出口 IP 重命名

- 启用「根据出口 IP 重命名」可获得准确的节点位置
- 测试完成后查询真实出口 IP（非服务器 IP）
- 需要确保 Mihomo 内核文件存在

### 支持的导入格式

- 订阅链接（base64 编码或 YAML 格式）
- YAML Clash 配置文件
- 节点链接（每行一个）
- YAML flow 格式：`{name: xxx, type: trojan, server: x.x.x.x, ...}`
